...
if len(sys.argv) > 1:
    ...
    with open(filename, "r+") as obj_file:
         ...
         markdown_file = obj_markdown_path + filename.replace("../../generated/", "").replace(".jsonld", ".txt")
         with open(obj_markdown_path + filename.replace("../../generated/", "").replace(".jsonld", ".txt"), "w+") as text_file:
             print("@startuml\n", file=text_file)
             graph(obj)
    ...
    for filename in glob.glob(os.path.join(folder_path, '*.jsonld')):
        with open(filename, "r+") as obj_file:
             obj = json.load(obj_file)
             markdown_file = markdown_path + filename.replace("../../generated/", "").replace(".jsonld", ".txt")
